...
import torch 
import os
from src.utils import load_model, natural_keys
import re 
...
class _ConvNetwork_LeNet(nn.Module):
    def __init__(self, input_size, output_size, q, args):
        super(_ConvNetwork_LeNet, self).__init__()
        self.args = args 
...
class _ConvNetwork_ResNet(nn.Module):
    def __init__(self, input_size, output_size,  q, args):
        super(_ConvNetwork_ResNet, self).__init__()
        self.args = args
        self.q = q
        self.in_planes = 24
        self.init_channels = input_size[1]
...
class Network(nn.Module):
  def __init__(self, input_size, output_size,  q, args, training_mode=True):
    super(Network, self).__init__()
    self.args = args
    self.input_size = 1
    self.q = q

    if args.model == "linear_sgld":
        self.template = _LinearNetwork
    elif args.model == "conv_lenet_sgld": 
        self.template = _ConvNetwork_LeNet
    elif args.model == "conv_resnet_sgld": 
        self.template = _ConvNetwork_ResNet
    else:
        raise NotImplementedError("Other templates not implemented!")

    for i in input_size:
        self.input_size*=int(i) 
    self.output_size = int(output_size)
    if training_mode:
        self.main_net = self.template(input_size, output_size, q, args)
    else:
        self.ensemble = nn.ModuleList([])
        for i in range(self.args.samples):
            model = self.template(input_size, output_size, q, args)
            self.ensemble.append(model)
        self.counter = 0
    self.training_mode = training_mode

  def load_ensemble(self, args, path=None, special_info=""):
      sample_names = []
      if path is None:
          path = args.save
      for root, dirs, files in os.walk(path):
        for filename in files:
          if ".pt" in filename:
            sample_name = re.findall('weights_'+special_info+'[0-9]*.pt', filename)
            if len(sample_name)>=1:
                sample_name = sample_name[0]
                sample_names.append(sample_name)
      sample_names.sort(key=natural_keys)
      self.sample_names = sample_names[-args.samples:]
      for i in range(args.samples):
        model = self.ensemble[i]
        load_model(model, path + "/" + self.sample_names[i])
        logging.info('### Loading model: {} ###'.format(path + "/" + self.sample_names[i]))

  def save_ensemble(self, args, path=None):
      if path is None:
          path = args.save
      for i in range(self.args.samples):
        logging.info('### Saving model: {} ###'.format(path + "/" + self.sample_names[i]))
        torch.save(self.ensemble[i].state_dict(), os.path.join(path, self.sample_names[i]))

  def fuse_model(self):
      if self.training_mode:
          self.main_net.fuse_model()
      else:
  ...
  def forward(self, x):
    if self.training_mode:
        ...
        x = self.ensemble[self.counter](x)
        self.counter+=1
