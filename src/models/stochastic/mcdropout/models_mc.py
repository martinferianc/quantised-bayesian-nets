...
from src.models.stochastic.mcdropout.dropout import BernoulliDropout
...
class LinearNetwork(nn.Module):
  def __init__(self, input_size, output_size, q, args):
    ...
    self.mu = nn.ModuleList([BernoulliDropout(self.args.p), nn.Linear(int(layers[len(layers)-1]), 1,  bias=True)])
    self.log_var = nn.ModuleList([BernoulliDropout(
        self.args.p), nn.Linear(int(layers[len(layers)-1]), 1,  bias=True)])
...
class BasicBlock(nn.Module):
    ...
    def fuse_model(self):
        torch.quantization.fuse_modules(self.stem, [['0', '1','2'], ['4', '5']], inplace=True)
