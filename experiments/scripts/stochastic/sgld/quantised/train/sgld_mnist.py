...
parser.add_argument('--model', type=str, default='conv_lenet_sgld', help='the model that we want to train')
...
def main():
  ...
  if not load:
    ...
    for i in range(args.samples):
      ...
      utils.load_model(model, args.load+"/"+sample_names[i], replace=False)
      logging.info('### Loading model: {} ###'.format(args.load+"/"+sample_names[i]))
